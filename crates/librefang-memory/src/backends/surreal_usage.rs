...
impl SurrealUsageStore {
    ...
    async fn sum_cost_async(&self, query: &str) -> LibreFangResult<f64> {
        let mut res = self
            .db
            .query(query)
            .await
    ...
    async fn sum_tokens_async(&self, query: &str) -> LibreFangResult<u64> {
        let mut res = self
            .db
            .query(query)
            .await
        ...
        if let Some(row) = rows.into_iter().next() {
            if let Some(v) = row.get("total") {
                return Ok(v.as_i64().unwrap_or(0) as u64);
            }
        }
        Ok(0)
    }
...
// ── Helpers for time window SQL fragments ─────────────────────────────────────
...
impl UsageBackend for SurrealUsageStore {
    ...
    fn query_hourly(&self, agent_id: AgentId) -> LibreFangResult<f64> {
        let agent = agent_id.0.to_string();
        let since = one_hour_ago();
        let q = format!(
            "SELECT math::sum(cost_usd) AS total FROM usage_events \
             WHERE agent_id = '{agent}' AND timestamp > '{since}'"
        );
        block_on(self.sum_cost_async(&q))
    }
    ...
    fn query_provider_tokens_hourly(&self, provider: &str) -> LibreFangResult<u64> {
        ...
        block_on(self.sum_tokens_async(&q))
    }

    fn cleanup_old(&self, days: u32) -> LibreFangResult<usize> {
        block_on(async {
            let cutoff = (Utc::now() - chrono::Duration::days(days as i64)).to_rfc3339();
            let q = format!("DELETE FROM usage_events WHERE timestamp < '{cutoff}'");
            let mut res = self
                .db
                .query(&q)
                .await
