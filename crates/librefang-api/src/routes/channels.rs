...
pub async fn configure_sidecar_channel(
    ...
    //
    //    BossFang: `schema` and `body` are moved into the blocking closure below, so the
    //    non-secret env map and the managed-key list the SurrealDB override write needs are
    //    captured HERE, before the move. Secret fields are deliberately excluded — they live
    //    in `secrets.env` and must never reach the config store.
    #[cfg(feature = "surreal-backend")]
    let (surreal_nonsecret_env, surreal_managed_env_keys): (
        ...
        for f in &schema.fields {
            ...
            managed.push(f.key.clone());
            if let Some(raw) = body.values.get(&f.key) {
                let trimmed = raw.trim();
                if !trimmed.is_empty() {
                    env.insert(f.key.clone(), trimmed.to_string());
                }
            }
        }
        (env, managed)
    ...
    // BossFang C-005d.3: persist the NON-secret `[[sidecar_channels]]` structure to the
    // SurrealDB config store so config.toml can stay read-only (K8s ConfigMap). The
    // blocking write above already handled secrets.env and, on the sqlite path, config.toml.
    // Validation runs BEFORE the store write, so an invalid edit returns 400 having written
    // nothing to the store. `(merged, raw)` is applied via `replace_config` below in place of
    // the legacy `reload_config` that re-reads config.toml.
    #[cfg(feature = "surreal-backend")]
    let surreal_apply: (librefang_types::config::KernelConfig, String) = {
        ...
        let _config_guard = state.config_write_lock.lock().await;
        let managed_env_keys: Vec<&str> =
            surreal_managed_env_keys.iter().map(String::as_str).collect();
        ...
        let (merged, raw) = resolve_config_with_overrides(&surreal_config_path, &overrides)
            .map_err(|e| {
