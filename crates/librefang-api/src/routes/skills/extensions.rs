...
// Upstream calls this around its inline `reload_config()` + `reload_mcp_servers()`
// pair. BossFang routes both through `super::mcp::apply_mcp_mutation`, which
// returns the error response itself (propagated at the `return resp;` call
// sites above), so there is no production caller left here — only the unit test
// that pins the scrubbed-response shape.
#[cfg(test)]
