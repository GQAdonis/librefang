...
enum Commands {
    ...
    /// MCP server management (stdio server + add/remove/list/catalog) [*].
    #[command(
    ...
    /// Add an MCP server (one-click catalog install). Alias for `mcp add`.
    #[command(
        long_about = "Add an MCP server by catalog id with one-click setup.\n\nAlias for `librefang mcp add <id>`. Installs and configures an MCP server\nfrom the catalog. Optionally provide an API key inline.\n\nExamples:\n  librefang add github                    # Interactive key prompt\n  librefang add slack --key xoxb-...      # Provide key inline\n  librefang add notion"
    )]
    Add {
        /// Catalog id (e.g., "github", "slack", "notion").
        name: String,
        /// API key or token to store in the vault.
        #[arg(long)]
        key: Option<String>,
    },
    /// Remove an installed MCP server. Alias for `mcp remove`.
    #[command(
        long_about = "Remove a configured MCP server by id.\n\nAlias for `librefang mcp remove <id>`.\n\nExamples:\n  librefang remove github\n  librefang remove slack"
    )]
    Remove {
        /// MCP server id.
        name: String,
    },
    /// List or search the MCP catalog. Alias for `mcp catalog`.
    #[command(
        long_about = "List or search MCP catalog entries.\n\nAlias for `librefang mcp catalog [query]`.\n\nExamples:\n  librefang integrations            # List all catalog entries\n  librefang integrations \"code\"     # Search for code-related entries"
    )]
    Integrations {
    ...
    #[command(
        long_about = "Scaffold a new skill or MCP server template.\n\nCreates boilerplate files for developing a custom skill or MCP server template.\n\nExamples:\n  librefang new skill   # Scaffold a new skill\n  librefang new mcp     # Scaffold a new MCP server template"
    )]
    New {
        /// What to scaffold.
        #[arg(value_enum)]
...
fn main() {
    ...
    match cli.command {
        ...
        Some(Commands::Mcp { command }) => match command {
            ...
            Some(McpCommands::Catalog { query }) => cmd_integrations_list(query.as_deref()),
            Some(McpCommands::Add { name, key }) => cmd_integration_add(&name, key.as_deref()),
            Some(McpCommands::Remove { name }) => cmd_integration_remove(&name),
        },
