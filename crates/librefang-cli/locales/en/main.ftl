...
channel-prompt-secret-retype =   { $label } ({ $key }) [set, required — retype to save]:
...
maintenance-service-install-system-hint = For a service that starts at boot rather than at login, use: sudo librefang service install --system
maintenance-service-system-needs-root = --system writes to /Library/LaunchDaemons, which requires root.
maintenance-service-system-sudo-hint = Re-run with sudo: sudo librefang service install --system
maintenance-service-system-macos-only = --system installs a macOS LaunchDaemon and is not implemented on this platform.
maintenance-service-system-linux-hint = On NixOS use the services.librefang module; on other Linux distributions write a system unit to /etc/systemd/system/ or keep the user service that `librefang service install` creates.
maintenance-service-system-no-sudo-user = Could not determine which account the daemon should run as: SUDO_USER is unset, so this looks like a direct root login rather than sudo.
maintenance-service-system-chown-failed = Failed to give { $user } ownership of { $path }: { $error }
maintenance-service-system-agent-conflict = A per-user LaunchAgent is already installed at { $path }. It carries the same ai.librefang.daemon label and the same state directory as the boot-time LaunchDaemon, so installing both would leave the agent failing and relaunching after every login.
maintenance-service-system-agent-conflict-fix = Remove it first, as your own user rather than through sudo: librefang service uninstall
maintenance-service-system-chmod-failed = Failed to set permissions on { $path }: { $error }
maintenance-launchdaemon-loaded = LaunchDaemon loaded — starts at boot and runs as { $user }
maintenance-launchdaemon-removed = Removed LaunchDaemon
maintenance-launchdaemon-remove-failed = Failed to remove the LaunchDaemon plist: { $error }
...
doctor-check-update-available-warn = Update available: { $current } -> { $latest } (see https://github.com/GQAdonis/librefang/releases)
...
desktop-install-skipped-manual =   Or download from https://github.com/GQAdonis/librefang/releases
...
desktop-install-download-manual = Download manually: https://github.com/GQAdonis/librefang/releases
...
everyapi-connect-models-fetch-unauthorized = The gateway rejected the relay key, so nothing was written.
everyapi-connect-models-fetch-unauthorized-fix = Run `everyapi login` to refresh the key, then re-run this command.
everyapi-connect-models-fetch-failed-would-clobber = Could not fetch the gateway's model list, and the existing provider entry already carries models; refusing to replace it with an empty one.
everyapi-connect-provider-rejected = The daemon rejected the provider definition: { $error }
everyapi-connect-pricing-fetch-failed = Could not fetch the gateway's pricing feed; registering models with no per-token price.
everyapi-connect-pricing-fetch-failed-fix = Check that the gateway is reachable, then re-run this command to fill in the pricing. Until then, spend on these models is not counted against any budget.
...
everyapi-connect-skip-no-context-window = { $model }: an output limit is known for this model but no context window is, and a text model without both is discarded when the catalog loads.
everyapi-connect-skip-no-output-limit = { $model }: a context window is known for this model but no output limit is, and a text model without both is discarded when the catalog loads.
everyapi-connect-token-limits-borrowed = The gateway publishes no context window and/or no output limit for these models, so the missing number was borrowed from the built-in OpenRouter snapshot and describes that copy of the model rather than this gateway's: { $models }
everyapi-connect-models-unpriced = These models carry no per-token price — they are absent from the gateway's pricing feed or billed per call — so their spend is recorded as unknown rather than free: { $models }
...
everyapi-connect-default-needs-daemon = The daemon is not running, so the default model was left unchanged. Start it and re-run `librefang models connect everyapi --set-default`.
everyapi-connect-default-url-pin-failed = Could not pin the gateway URL into config.toml; the default model was still set, but a restarted daemon may not resolve it.
...
doctor-audit-desktop-deps-ok = Desktop GTK/WebKit stack present — pkg-config resolves { $module }.
...
doctor-audit-desktop-deps-hint-apt = On { $distro }, find the packages that provide { $modules } with `apt-cache search webkit2gtk`, then install them with `sudo apt install <package>`. Package names differ between releases, so search rather than assume.
doctor-audit-desktop-deps-hint-pacman = On { $distro }, find the packages that provide { $modules } with `pacman -Ss webkit2gtk`, then install them with `sudo pacman -S <package>`. Package names differ between releases, so search rather than assume.
doctor-audit-desktop-deps-hint-dnf = On { $distro }, find the packages that provide { $modules } with `dnf search webkit2gtk`, then install them with `sudo dnf install <package>`. Package names differ between releases, so search rather than assume.
...
doctor-audit-desktop-deps-hint-generic = On { $distro }, install the GTK 3 and WebKitGTK development packages your package manager provides, until pkg-config resolves { $modules }.
...
tui-event-promote-missing-pr-url = Missing PR URL in response
tui-event-promote-http-error = { $status }: { $detail }
tui-event-promote-not-available-in-process = Promote is only available when connected to a daemon
...
tui-event-backup-delete-failed = Failed to delete backup { $filename }
tui-event-backup-restore-failed = Restore from { $filename } failed
...
tui-mod-backup-created = Backup created: { $filename }
tui-mod-backup-deleted = Backup deleted: { $filename }
tui-mod-backup-restored = Restored { $files } files from { $filename }. Restart the daemon for all changes to take effect.
tui-mod-backup-restored-with-errors = Restored { $files } files from { $filename }, { $errors } failed. Restart the daemon for all changes to take effect.
...
tui-templates-hints =   [↑↓] Navigate  [Enter] Spawn Agent  [f] Filter Category  [p] Promote  [r] Refresh
tui-templates-promote-only-custom = Only custom agent types can be promoted to the registry
tui-templates-promoting = Promoting '{ $name }' to the registry…
tui-templates-promoted = Registry PR opened: { $url }
tui-templates-promote-failed = Could not promote '{ $name }': { $error }
tui-templates-provider-not-configured = Provider '{ $provider }' not configured. Set API key in Settings first.
tui-templates-manifest-unavailable = Could not load the manifest for '{ $name }'. Check the daemon log.
...
tui-settings-tab-backups = 4 Backups
tui-settings-hints-backups =   [↑↓] Navigate  [c] Create  [Enter] Restore  [d] Delete  [r] Refresh
tui-settings-hints-restore =   [↑↓] Move  [Space] Toggle  [Enter] Restore  [Esc] Cancel
tui-settings-backups-header-filename = Archive
tui-settings-backups-header-size = Size
tui-settings-backups-header-created = Created
tui-settings-backups-header-components = Components
tui-settings-backups-loading = Loading backups…
tui-settings-backups-empty = No backups yet. Press [c] to create one.
tui-settings-backups-creating = Creating backup…
tui-settings-backups-delete-confirm =   Delete this backup? [y] Yes  [any] Cancel
tui-settings-backups-restore-title = Restore { $filename }
tui-settings-backups-restore-warning = Restore overwrites existing state. Restart the daemon afterwards.
...
tui-memory-config-saved = Saved
tui-memory-config-save-failed = Save failed: { $error }
tui-memory-config-unsaved = Unsaved changes — press s to save, Esc to discard
tui-memory-config-save-partial = Saved to disk, but the live reload failed — a restart is required
tui-memory-config-hint =   j/k select · space/Enter toggle or edit · s save · r refresh · Esc back
...
tui-goals-label-status = Status:
tui-goals-label-description = Description:
tui-goals-label-agent = Agent:
tui-goals-label-progress = Progress:
tui-goals-label-phase = Run phase:
tui-goals-label-iteration = Iteration:
tui-goals-label-title = Title:
...
tui-goals-step = step { $n }/{ $total }
tui-goals-title-hint = A short name for the goal, for example "Fix the login timeout".
tui-goals-description-hint = What the agent should accomplish. This becomes its prompt.
tui-goals-agent-hint = Name or UUID of the agent that will pursue the goal. Required — a goal with no agent cannot run.
tui-goals-nav-next = [Enter] Next  [Esc] Back
tui-goals-nav-submit = [Enter] Create  [Esc] Back
tui-goals-create-incomplete = A title and an agent are both required.
...
tui-goals-run-running = running
tui-goals-run-paused = paused
tui-goals-run-finished = finished
tui-goals-run-max-iterations = iteration cap reached
tui-goals-run-rate-limited = stopped by the rate limiter
tui-goals-run-stopped = stopped by an operator
...
# --- goal command (CLI) ---
cmd-goal-agent-required = A goal needs an agent to pursue it.
cmd-goal-agent-required-fix = Pass --agent <name-or-uuid>. Run `librefang agents list` to see the available agents.
cmd-goal-agent-unknown = No agent matches '{ $agent }'.
cmd-goal-created = Goal created: { $id }
cmd-goal-create-error = Error creating the goal: { $error }
cmd-goal-start-error = Error starting the run: { $error }
cmd-goal-watching = Watching the run. Ctrl+C stops watching; the run itself keeps going.
cmd-goal-progress = [{ $current }/{ $total }] { $phase } — { $progress }% done
...
tui-channels-title-screen = Channels
tui-channels-header-instance = INSTANCE
tui-channels-header-agent = AGENT
tui-channels-header-status = STATUS
tui-channels-header-traffic = IN/OUT
tui-channels-loading = Loading channel instances…
tui-channels-empty-state = No channel instances configured. Add one with [a].
tui-channels-agent-unbound = (none)
tui-channels-group-count = { " " }({ $count })
tui-channels-add-new-option =   + Add a channel instance
tui-channels-status-online = ● Online
tui-channels-status-degraded = ◐ Degraded
tui-channels-status-stopped = ○ Stopped
tui-channels-status-unsupervised = ○ Not started
tui-channels-confirm-delete =   Delete channel instance { $name }? [y/n]
tui-channels-hints-list =   [↑↓] Navigate  [Enter/e] Edit  [a] Add  [d] Delete  [r] Refresh  [R] Reload adapters
tui-channels-title-picker = Pick the adapter the new instance runs on
tui-channels-picker-empty = No sidecar adapters available. Install the Python SDK, then reload.
tui-channels-no-schema = no schema — configure by hand
tui-channels-hints-picker =   [↑↓] Navigate  [Enter] Select  [Esc] Back
tui-channels-title-create = Add channel instance
tui-channels-title-edit = Edit channel instance
tui-channels-label-instance-name = Instance name
tui-channels-label-instance-name-locked = Instance name (fixed)
tui-channels-label-agent = Default agent
tui-channels-placeholder-agent = (no agent bound)
tui-channels-secret-stored = (stored — retype to save)
tui-channels-hints-form =   [↑↓] Field  [Space/←→] Toggle  [Enter] Save  [Esc] Cancel
tui-channels-error-no-adapter = Pick an adapter before saving.
tui-channels-error-name-required = The instance name cannot be empty.
tui-channels-error-name-invalid = { $name } is not a usable instance name — use letters, digits, `-`, `_` or `.` only.
tui-channels-error-name-taken = An instance named { $name } already exists.
tui-channels-error-name-secret-collision = { $name } and the existing { $other } both map to the secret prefix { $prefix }__, so they would share each other's secrets. Pick a name that differs by more than punctuation.
tui-channels-error-field-required = { $label } is required.
tui-channels-error-secret-required = { $label } is stored but cannot be read back — retype it to save.
tui-mod-channel-saved = Channel instance { $name } saved.
tui-mod-channel-saved-restart-required = Channel instance { $name } saved — restart the daemon to apply.
tui-mod-channel-saved-shadowed = Channel instance { $name } saved, but { $keys } is set in the daemon's environment and out-ranks it — unset it and restart.
tui-mod-channel-deleted = Channel instance { $name } deleted.
tui-mod-channels-reloaded = Channels reloaded ({ $started } started).
tui-event-channel-instances-fetch-failed = Failed to load channel instances: { $error }
tui-event-aux-fetch-failed = Failed to load auxiliary LLM chains
tui-event-aux-save-failed = Failed to save the auxiliary chain for { $task }
tui-event-aux-save-not-available-in-process = Auxiliary chain editing is not available in in-process mode
tui-event-aux-tasks-fetch-failed = Failed to load the auxiliary task list: { $error }
tui-event-aux-tasks-schema-missing = The daemon's config schema carried no auxiliary task list; showing only the tasks already configured.
tui-event-channels-not-available-in-process = Channel management needs a running daemon.
