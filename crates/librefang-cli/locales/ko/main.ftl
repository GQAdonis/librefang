...
channel-prompt-secret-retype =   { $label } ({ $key }) [설정됨, 필수 — 저장하려면 다시 입력]:
...
doctor-audit-desktop-deps-ok = 데스크톱 GTK/WebKit 스택이 준비됨 — pkg-config가 { $module }을(를) 확인했습니다.
...
doctor-audit-desktop-deps-hint-apt = { $distro }에서는 `apt-cache search webkit2gtk`로 { $modules }을(를) 제공하는 패키지를 찾은 뒤 `sudo apt install <package>`로 설치하십시오. 패키지 이름은 릴리스마다 다르므로 추측하지 말고 검색하십시오.
doctor-audit-desktop-deps-hint-pacman = { $distro }에서는 `pacman -Ss webkit2gtk`로 { $modules }을(를) 제공하는 패키지를 찾은 뒤 `sudo pacman -S <package>`로 설치하십시오. 패키지 이름은 릴리스마다 다르므로 추측하지 말고 검색하십시오.
doctor-audit-desktop-deps-hint-dnf = { $distro }에서는 `dnf search webkit2gtk`로 { $modules }을(를) 제공하는 패키지를 찾은 뒤 `sudo dnf install <package>`로 설치하십시오. 패키지 이름은 릴리스마다 다르므로 추측하지 말고 검색하십시오.
...
doctor-audit-desktop-deps-hint-generic = { $distro }에서는 패키지 관리자가 제공하는 GTK 3 및 WebKitGTK 개발 패키지를 pkg-config가 { $modules }을(를) 확인할 수 있을 때까지 설치하십시오.
...
tui-event-promote-missing-pr-url = 응답에 PR URL이 없습니다
tui-event-promote-http-error = { $status }: { $detail }
tui-event-promote-not-available-in-process = 승격은 데몬에 연결된 경우에만 사용할 수 있습니다
...
tui-event-backup-delete-failed = 백업 { $filename }을(를) 삭제하지 못했습니다
tui-event-backup-restore-failed = { $filename }에서 복원하지 못했습니다
...
tui-mod-backup-created = 백업이 생성되었습니다: { $filename }
tui-mod-backup-deleted = 백업이 삭제되었습니다: { $filename }
tui-mod-backup-restored = { $filename }에서 { $files }개 파일을 복원했습니다. 모든 변경 사항을 적용하려면 데몬을 재시작하세요.
tui-mod-backup-restored-with-errors = { $filename }에서 { $files }개 파일을 복원했고 { $errors }개는 실패했습니다. 모든 변경 사항을 적용하려면 데몬을 재시작하세요.
...
tui-templates-promote-only-custom = 사용자 지정 에이전트 타입만 레지스트리로 승격할 수 있습니다
tui-templates-promoting = '{ $name }'을(를) 레지스트리로 승격하는 중…
tui-templates-promoted = 레지스트리 PR이 열렸습니다: { $url }
tui-templates-promote-failed = '{ $name }'을(를) 승격할 수 없습니다: { $error }
tui-templates-provider-not-configured = 프로바이더 '{ $provider }'이(가) 구성되지 않았습니다. 먼저 설정에서 API 키를 설정하십시오.
tui-templates-manifest-unavailable = '{ $name }'의 매니페스트를 불러올 수 없습니다. 데몬 로그를 확인하십시오.
...
tui-settings-tab-backups = 4 백업
tui-settings-hints-backups =   [↑↓] 탐색  [c] 생성  [Enter] 복원  [d] 삭제  [r] 새로고침
tui-settings-hints-restore =   [↑↓] 이동  [Space] 선택 전환  [Enter] 복원  [Esc] 취소
tui-settings-backups-header-filename = 아카이브
tui-settings-backups-header-size = 크기
tui-settings-backups-header-created = 생성 시각
tui-settings-backups-header-components = 구성 요소
tui-settings-backups-loading = 백업을 불러오는 중…
tui-settings-backups-empty = 아직 백업이 없습니다. [c]를 눌러 생성하세요.
tui-settings-backups-creating = 백업을 생성하는 중…
tui-settings-backups-delete-confirm =   이 백업을 삭제하시겠습니까? [y] 예  [any] 취소
tui-settings-backups-restore-title = { $filename } 복원
tui-settings-backups-restore-warning = 복원은 기존 상태를 덮어씁니다. 완료 후 데몬을 재시작하세요.
tui-settings-backups-restore-keep-config = 이 머신의 config.toml 유지 (클론 모드)
tui-settings-backups-restore-all = 아카이브의 모든 항목이 선택되었습니다.
tui-settings-backups-restore-none = 구성 요소를 하나 이상 선택하거나 [Esc]로 취소하세요.
tui-settings-tab-auxiliary = 5 보조 작업
tui-settings-hints-auxiliary =   [↑↓] 탐색  [Enter] 체인 편집  [r] 새로고침  [Esc] 취소
tui-settings-auxiliary-header-task = 작업
tui-settings-auxiliary-header-chain = 대체 체인
tui-settings-auxiliary-loading = 보조 작업 구성을 불러오는 중…
tui-settings-auxiliary-empty = 구성된 보조 작업이 없습니다.
tui-settings-auxiliary-not-configured = (구성되지 않음)
tui-settings-auxiliary-editing = { $task }의 체인 편집 (쉼표로 구분된 provider:model):
# chat.rs
...
tui-workflows-param-required = { $name } 은(는) 필수입니다
tui-workflows-params-none = 이 워크플로는 선언된 매개변수가 없습니다 - 입력이 자유 텍스트로 전송됩니다.
tui-workflows-params-failed = 선언된 매개변수를 불러오지 못했습니다 - 입력이 자유 텍스트로 전송됩니다.
tui-workflows-hints-run-input =   [Enter] 실행  [Tab/↑↓] 필드  [Esc] 취소
...
tui-memory-config-saved = 저장됨
tui-memory-config-save-failed = 저장 실패: { $error }
tui-memory-config-unsaved = 저장되지 않은 변경 사항 — s로 저장, Esc로 취소
tui-memory-config-save-partial = 디스크에는 저장되었지만 실시간 재적용에 실패했습니다 — 재시작이 필요합니다
tui-memory-config-hint =   j/k 이동 · space/Enter 전환 또는 편집 · s 저장 · r 새로고침 · Esc 뒤로
...
tui-goals-label-status = 상태:
tui-goals-label-description = 설명:
tui-goals-label-agent = 에이전트:
tui-goals-label-progress = 진행률:
tui-goals-label-phase = 실행 단계:
tui-goals-label-iteration = 반복:
tui-goals-label-title = 제목:
...
tui-goals-step = 단계 { $n }/{ $total }
tui-goals-title-hint = 목표의 짧은 이름. 예: "로그인 시간 초과 수정".
tui-goals-description-hint = 에이전트가 달성해야 할 내용. 그대로 프롬프트가 됩니다.
tui-goals-agent-hint = 목표를 수행할 에이전트의 이름 또는 UUID. 필수 — 에이전트가 없는 목표는 실행할 수 없습니다.
...
tui-goals-run-max-iterations = 반복 한도 도달
tui-goals-run-rate-limited = 속도 제한으로 중지됨
tui-goals-run-stopped = 운영자가 중지함
...
# --- goal command (CLI) ---
cmd-goal-agent-required = 목표를 수행할 에이전트가 필요합니다.
cmd-goal-agent-required-fix = --agent <이름-또는-UUID>를 지정하세요. `librefang agents list`로 사용 가능한 에이전트를 확인할 수 있습니다.
cmd-goal-agent-unknown = '{ $agent }'과(와) 일치하는 에이전트가 없습니다.
cmd-goal-created = 목표 생성됨: { $id }
cmd-goal-create-error = 목표 생성 오류: { $error }
cmd-goal-start-error = 실행 시작 오류: { $error }
cmd-goal-watching = 실행을 관찰하는 중입니다. Ctrl+C는 관찰만 멈추며 실행은 계속됩니다.
cmd-goal-progress = [{ $current }/{ $total }] { $phase } — { $progress }% 완료
...
tui-tab-channels = 채널
tui-channels-title-screen = 채널
tui-channels-header-instance = 인스턴스
tui-channels-header-agent = 에이전트
tui-channels-header-status = 상태
tui-channels-header-traffic = 수신/발신
tui-channels-loading = 채널 인스턴스를 불러오는 중…
tui-channels-empty-state = 구성된 채널 인스턴스가 없습니다. [a]로 추가하세요.
tui-channels-agent-unbound = (없음)
tui-channels-group-count = { " " }({ $count })
tui-channels-add-new-option =   + 채널 인스턴스 추가
tui-channels-status-online = ● 온라인
tui-channels-status-degraded = ◐ 저하됨
tui-channels-status-stopped = ○ 중지됨
tui-channels-status-unsupervised = ○ 시작되지 않음
tui-channels-confirm-delete =   채널 인스턴스 { $name }을(를) 삭제하시겠습니까? [y/n]
tui-channels-hints-list =   [↑↓] 이동  [Enter/e] 편집  [a] 추가  [d] 삭제  [r] 새로고침  [R] 어댑터 재로드
tui-channels-title-picker = 새 인스턴스가 실행될 어댑터를 선택하세요
tui-channels-picker-empty = 사용 가능한 사이드카 어댑터가 없습니다. Python SDK를 설치한 뒤 다시 로드하세요.
tui-channels-no-schema = 스키마 없음 — 수동으로 구성하세요
tui-channels-hints-picker =   [↑↓] 이동  [Enter] 선택  [Esc] 뒤로
tui-channels-title-create = 채널 인스턴스 추가
tui-channels-title-edit = 채널 인스턴스 편집
tui-channels-label-instance-name = 인스턴스 이름
tui-channels-label-instance-name-locked = 인스턴스 이름 (변경 불가)
tui-channels-label-agent = 기본 에이전트
tui-channels-placeholder-agent = (연결된 에이전트 없음)
tui-channels-secret-stored = (저장됨 — 저장하려면 다시 입력)
tui-channels-hints-form =   [↑↓] 필드  [Space/←→] 전환  [Enter] 저장  [Esc] 취소
tui-channels-error-no-adapter = 저장하기 전에 어댑터를 선택하세요.
tui-channels-error-name-required = 인스턴스 이름은 비워 둘 수 없습니다.
tui-channels-error-name-invalid = { $name }은(는) 사용할 수 없는 인스턴스 이름입니다 — 영숫자와 `-`, `_`, `.`만 사용하세요.
tui-channels-error-name-taken = { $name } 이름의 인스턴스가 이미 있습니다.
tui-channels-error-name-secret-collision = { $name }와(과) 기존 { $other }은(는) 모두 시크릿 접두사 { $prefix }__로 변환되어 서로의 시크릿을 공유하게 됩니다. 문장 부호 이상으로 다른 이름을 선택하세요.
tui-channels-error-field-required = { $label }은(는) 필수입니다.
tui-channels-error-secret-required = { $label }은(는) 저장되어 있지만 다시 읽을 수 없습니다 — 저장하려면 다시 입력하세요.
tui-mod-channel-saved = 채널 인스턴스 { $name } 저장됨.
tui-mod-channel-saved-restart-required = 채널 인스턴스 { $name } 저장됨 — 적용하려면 데몬을 재시작하세요.
tui-mod-channel-saved-shadowed = 채널 인스턴스 { $name } 저장됨. 단 { $keys }이(가) 데몬 환경에 설정되어 우선하므로, 해제한 뒤 재시작하세요.
tui-mod-channel-deleted = 채널 인스턴스 { $name } 삭제됨.
tui-mod-channels-reloaded = 채널이 재로드되었습니다({ $started }개 시작).
tui-event-channel-instances-fetch-failed = 채널 인스턴스를 불러오지 못했습니다: { $error }
tui-event-aux-fetch-failed = 보조 LLM 체인을 불러오지 못했습니다
tui-event-aux-save-failed = { $task }의 보조 체인을 저장하지 못했습니다
tui-event-aux-save-not-available-in-process = 인프로세스 모드에서는 보조 체인 편집을 사용할 수 없습니다
tui-event-aux-tasks-fetch-failed = 보조 작업 목록을 불러오지 못했습니다: { $error }
tui-event-aux-tasks-schema-missing = 데몬의 구성 스키마에 보조 작업 목록이 없습니다. 이미 구성된 작업만 표시합니다.
tui-event-channels-not-available-in-process = 채널 관리에는 실행 중인 데몬이 필요합니다.
