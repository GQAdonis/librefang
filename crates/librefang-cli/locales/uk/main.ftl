...
channel-prompt-secret-retype =   { $label } ({ $key }) [встановлено, обовʼязкове — введіть заново, щоб зберегти]:
...
agent-set-unknown-field = Невідоме поле: { $field }. Підтримувані поля: model, temperature, max_tokens, top_p, frequency_penalty, presence_penalty, context_window, max_output_tokens
agent-set-field-success = Для агента { $id } поле { $field } встановлено у { $value }.
...
doctor-check-update-available-warn = Доступне оновлення: { $current } -> { $latest } (див. https://github.com/GQAdonis/librefang/releases)
...
desktop-install-skipped-manual =   Або завантажте з https://github.com/GQAdonis/librefang/releases
...
desktop-install-download-manual = Завантажте вручну: https://github.com/GQAdonis/librefang/releases
...
doctor-audit-desktop-deps-ok = Стек GTK/WebKit для стільниці наявний — pkg-config знаходить { $module }.
...
doctor-audit-desktop-deps-hint-apt = У { $distro } знайдіть пакунки, що надають { $modules }, за допомогою `apt-cache search webkit2gtk`, а потім встановіть їх через `sudo apt install <package>`. Назви пакунків різняться між випусками, тому шукайте, а не вгадуйте.
doctor-audit-desktop-deps-hint-pacman = У { $distro } знайдіть пакунки, що надають { $modules }, за допомогою `pacman -Ss webkit2gtk`, а потім встановіть їх через `sudo pacman -S <package>`. Назви пакунків різняться між випусками, тому шукайте, а не вгадуйте.
doctor-audit-desktop-deps-hint-dnf = У { $distro } знайдіть пакунки, що надають { $modules }, за допомогою `dnf search webkit2gtk`, а потім встановіть їх через `sudo dnf install <package>`. Назви пакунків різняться між випусками, тому шукайте, а не вгадуйте.
...
doctor-audit-desktop-deps-hint-generic = У { $distro } встановіть пакунки розробки GTK 3 та WebKitGTK, які надає ваш менеджер пакунків, доки pkg-config не почне знаходити { $modules }.
...
chat-runner-goal-daemon-unsupported = /goal ще не підтримується під час підключення до демона — створіть і запустіть ціль з панелі приладів.
...
tui-event-promote-missing-pr-url = Відсутній URL PR у відповіді
tui-event-promote-http-error = { $status }: { $detail }
tui-event-promote-not-available-in-process = Просування доступне лише при підключенні до демона
...
tui-templates-promote-only-custom = До реєстру можна просувати лише власні типи агентів
tui-templates-promoting = Просування '{ $name }' до реєстру…
tui-templates-promoted = PR реєстру відкрито: { $url }
tui-templates-promote-failed = Не вдалося просунути '{ $name }': { $error }
tui-templates-provider-not-configured = Провайдер '{ $provider }' не налаштований. Спочатку встановіть API-ключ у Налаштуваннях.
tui-templates-manifest-unavailable = Не вдалося завантажити маніфест для '{ $name }'. Перевірте журнал демона.
...
tui-settings-tab-auxiliary = 5 Допоміжні завдання
tui-settings-hints-auxiliary =   [↑↓] Навігація  [Enter] Редагувати ланцюжок  [r] Оновити  [Esc] Скасувати
tui-settings-auxiliary-header-task = Завдання
tui-settings-auxiliary-header-chain = Резервний ланцюжок
tui-settings-auxiliary-loading = Завантаження конфігурації допоміжних завдань…
tui-settings-auxiliary-empty = Допоміжних завдань не налаштовано.
tui-settings-auxiliary-not-configured = (не налаштовано)
tui-settings-auxiliary-editing = Редагування ланцюжка для { $task } (provider:model через кому):
# chat.rs
...
tui-workflows-param-required = { $name } є обов'язковим
tui-workflows-params-none = Цей воркфлоу не оголошує параметрів - вхідні дані надсилаються як вільний текст.
tui-workflows-params-failed = Не вдалося завантажити оголошені параметри - вхідні дані надсилаються як вільний текст.
tui-workflows-hints-run-input =   [Enter] Запуск  [Tab/↑↓] Поля  [Esc] Скасувати
...
tui-memory-config-saved = Збережено
tui-memory-config-save-failed = Помилка збереження: { $error }
tui-memory-config-unsaved = Незбережені зміни — натисніть s щоб зберегти, Esc щоб скасувати
tui-memory-config-save-partial = Збережено на диск, але оновлення на льоту не вдалося — потрібен перезапуск
tui-memory-config-hint =   j/k вибір · space/Enter перемкнути або редагувати · s зберегти · r оновити · Esc назад
...
tui-mod-agent-model-params-updated = Параметри моделі оновлено для агента { $id }.
tui-agents-param-temperature = Температура
tui-agents-param-temperature-hint = temperature — наскільки модель варіює формулювання. Менше — стабільніше.
tui-agents-param-top-p = Top-p
tui-agents-param-top-p-hint = top_p — nucleus sampling. Зазвичай залишають успадкованим.
tui-agents-param-frequency-penalty = Штраф за частоту
tui-agents-param-frequency-penalty-hint = frequency_penalty — стримує повторення тих самих слів.
tui-agents-param-presence-penalty = Штраф за присутність
tui-agents-param-presence-penalty-hint = presence_penalty — стримує повернення до тих самих тем.
tui-agents-param-max-tokens = Довжина відповіді
tui-agents-param-max-tokens-hint = max_tokens — яку довжину відповіді просить цей агент. Ваше значення має перевагу над моделлю.
tui-agents-param-context-window = Вікно контексту
tui-agents-param-context-window-hint = context_window — скільки цей endpoint може прочитати. Це ліміт, а не налаштування.
tui-agents-param-max-output-tokens = Ліміт виводу
tui-agents-param-max-output-tokens-hint = max_output_tokens — власний ліміт виводу endpoint. Це ліміт, а не налаштування.
tui-agents-param-not-a-number = { $field } має бути числом
tui-agents-param-not-whole = { $field } має бути цілим числом більшим за 0
tui-agents-param-out-of-range = { $field } має бути між { $min } та { $max }
agent-set-invalid-integer = { $field } має бути додатним цілим числом, отримано '{ $value }'
agent-set-invalid-decimal = { $field } має бути десятковим числом, отримано '{ $value }'
agent-set-limit-warning = попередження: { $message }
...
purge-workspace-shared =   - ПРИМІТКА: каталог робочого простору залишено без змін. Він відповідає самому кореню робочих просторів або каталогу, на який також посилається маніфест іншого зареєстрованого агента — видалення знищило б більше, ніж дані цього агента; перевірте каталог робочих просторів самостійно.
purge-removed-agent-type =   - шаблон типу агента
purge-removed-cron-jobs =   - завдання cron
purge-removed-trigger-jobs =   - тригери подій
purge-removed-channel-bindings =   - прив'язки маршрутизації каналів/бесід
purge-other-orphans-present =   - ПРИМІТКА: у цій інсталяції є осиротілі записи під ідентифікатором, який не вдалося прив'язати до жодного імені (найімовірніше, дочірній або ефемерний агент, чий запис у реєстрі вже зник); їх залишено без змін.
...
purge-dry-run-header = Пробний запуск — буде очищено '{ $agent }' (відкриття бази даних все ще може застосувати відкладені міграції схеми та переіндексувати пошук, навіть якщо нічого не видаляється):
purge-failed-daemon-running = Очищення перервано: демон запущено на { $url } і утримує дані цієї інсталяції. Спочатку зупиніть його, або передайте --force, якщо ви впевнені.

# --- Goals screen (TUI) ---
tui-goals-title = ⌖ Цілі
tui-goals-count = Цілей: { $count }
...
tui-goals-label-status = Статус:
tui-goals-label-description = Опис:
tui-goals-label-agent = Агент:
tui-goals-label-progress = Прогрес:
tui-goals-label-phase = Фаза запуску:
tui-goals-label-iteration = Ітерація:
tui-goals-label-title = Назва:
tui-goals-hint-stop = [s] зупинити запуск
tui-goals-hint-start = [s] почати запуск
tui-goals-detail-hints = [q/Esc] Закрити  { $run_hint }  [r] Оновити
tui-goals-new-title = Нова ціль
tui-goals-step = крок { $n }/{ $total }
tui-goals-title-hint = Коротка назва цілі, наприклад "Виправити тайм-аут входу".
tui-goals-description-hint = Що має зробити агент. Це стає його промптом.
tui-goals-agent-hint = Ім'я або UUID агента, який виконуватиме ціль. Обов'язково — ціль без агента не запуститься.
tui-goals-nav-next = [Enter] Далі  [Esc] Назад
tui-goals-nav-submit = [Enter] Створити  [Esc] Назад
tui-goals-create-incomplete = Потрібні і назва, і агент.
tui-goals-phase-actv = ● АКТВ
tui-goals-phase-done = ○ ГОТОВО
tui-goals-phase-canc = ○ СКАС
tui-goals-phase-fail = ● ПОМИЛКА
tui-goals-phase-pend = ○ ОЧІК
tui-goals-phase-paused = ⏸ ПАУЗ
tui-goals-phase-rate-limited = ⚡ ЛІМІТ
tui-goals-phase-max-iterations = ⏱ МАКС
tui-goals-run-running = виконується
tui-goals-run-paused = призупинено
tui-goals-run-finished = завершено
tui-goals-run-max-iterations = досягнуто ліміт ітерацій
tui-goals-run-rate-limited = зупинено через ліміт запитів
tui-goals-run-stopped = зупинено оператором
tui-goal-created = Ціль створено: { $id }
...
# --- goal command (CLI) ---
cmd-goal-agent-required = Цілі потрібен агент, який її виконає.
cmd-goal-agent-required-fix = Передайте --agent <ім'я-або-uuid>. Виконайте `librefang agents list`, щоб побачити доступних агентів.
cmd-goal-agent-unknown = Жоден агент не відповідає '{ $agent }'.
cmd-goal-created = Ціль створено: { $id }
cmd-goal-create-error = Помилка створення цілі: { $error }
cmd-goal-start-error = Помилка запуску: { $error }
cmd-goal-watching = Спостереження за запуском. Ctrl+C зупиняє лише спостереження, сам запуск триває.
cmd-goal-progress = [{ $current }/{ $total }] { $phase } — виконано { $progress }%
cmd-goal-finished = Ціль успішно завершено.
...
tui-channels-header-instance = ЕКЗЕМПЛЯР
tui-channels-header-agent = АГЕНТ
tui-channels-header-status = СТАТУС
tui-channels-header-traffic = ВХ/ВИХ
tui-channels-loading = Завантаження екземплярів каналів…
tui-channels-empty-state = Немає налаштованих екземплярів каналів. Додайте через [a].
tui-channels-agent-unbound = (немає)
tui-channels-group-count = { " " }({ $count })
tui-channels-add-new-option =   + Додати екземпляр каналу
tui-channels-status-online = ● Онлайн
tui-channels-status-degraded = ◐ Погіршено
tui-channels-status-stopped = ○ Зупинено
tui-channels-status-unsupervised = ○ Не запущено
tui-channels-confirm-delete =   Видалити екземпляр каналу { $name }? [y/n]
tui-channels-hints-list =   [↑↓] Навігація  [Enter/e] Редагувати  [a] Додати  [d] Видалити  [r] Оновити  [R] Перезавантажити адаптери
tui-channels-title-picker = Виберіть адаптер, на якому працюватиме новий екземпляр
tui-channels-picker-empty = Немає доступних sidecar-адаптерів. Встановіть Python SDK та перезавантажте.
tui-channels-no-schema = немає схеми — налаштуйте вручну
tui-channels-hints-picker =   [↑↓] Навігація  [Enter] Вибрати  [Esc] Назад
tui-channels-title-create = Додати екземпляр каналу
tui-channels-title-edit = Редагувати екземпляр каналу
tui-channels-label-instance-name = Назва екземпляра
tui-channels-label-instance-name-locked = Назва екземпляра (незмінна)
tui-channels-label-agent = Типовий агент
tui-channels-placeholder-agent = (агент не привʼязаний)
tui-channels-secret-stored = (збережено — введіть заново, щоб зберегти)
tui-channels-hints-form =   [↑↓] Поле  [Space/←→] Перемкнути  [Enter] Зберегти  [Esc] Скасувати
tui-channels-error-no-adapter = Виберіть адаптер перед збереженням.
tui-channels-error-name-required = Назва екземпляра не може бути порожньою.
tui-channels-error-name-invalid = { $name } не є допустимою назвою екземпляра — використовуйте лише літери, цифри, `-`, `_` або `.`.
tui-channels-error-name-taken = Екземпляр з назвою { $name } вже існує.
tui-channels-error-name-secret-collision = { $name } та наявний { $other } зводяться до одного префікса секретів { $prefix }__, тож вони ділили б секрети одне одного. Оберіть назву, що відрізняється не лише пунктуацією.
tui-channels-error-field-required = { $label } є обовʼязковим.
tui-channels-error-secret-required = { $label } збережено, але його не можна прочитати — введіть заново, щоб зберегти.
tui-mod-channel-saved = Екземпляр каналу { $name } збережено.
tui-mod-channel-saved-restart-required = Екземпляр каналу { $name } збережено — перезапустіть демон, щоб застосувати.
tui-mod-channel-saved-shadowed = Екземпляр каналу { $name } збережено, але { $keys } задано в середовищі демона й має вищий приоритет — приберіть змінну та перезапустіть.
tui-mod-channel-deleted = Екземпляр каналу { $name } видалено.
tui-mod-channels-reloaded = Канали перезавантажено (запущено { $started }).
tui-event-channel-instances-fetch-failed = Не вдалося завантажити екземпляри каналів: { $error }
tui-event-aux-fetch-failed = Не вдалося завантажити допоміжні ланцюжки LLM
tui-event-aux-save-failed = Не вдалося зберегти допоміжний ланцюжок для { $task }
tui-event-aux-save-not-available-in-process = Редагування допоміжних ланцюжків недоступне в інпроцес-режимі
tui-event-aux-tasks-fetch-failed = Не вдалося завантажити список допоміжних завдань: { $error }
tui-event-aux-tasks-schema-missing = Схема конфігурації демона не містить списку допоміжних завдань; показано лише вже налаштовані завдання.
tui-event-channels-not-available-in-process = Керування каналами вимагає запущеного демона.
