...
impl CodexCliDriver {
    ...
    fn model_flag(model: &str) -> Option<String> {
        ...
        // A bare provider id or empty string means "no explicit model" — return
        // None so the CLI uses the model from its own ~/.codex/config.toml.
        if stripped.is_empty() || stripped == "codex-cli" {
            return None;
        }
        // "codex-latest" is a rolling alias that always points to the newest
        // Codex CLI model; currently resolves to gpt-5.5.
