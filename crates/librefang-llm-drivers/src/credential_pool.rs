...
pub struct CredentialPool {
    /// Guarded credential list, sorted by priority (descending) on construction.
    credentials: Mutex<Vec<PooledCredential>>,
    strategy: PoolStrategy,
    /// Monotonically incrementing index used by `RoundRobin`.
...
impl CredentialPool {
    ...
    pub fn acquire(&self) -> Option<String> {
        ...
        match self.strategy {
            ...
            PoolStrategy::RoundRobin => {
                let start = self.round_robin_idx.load(Ordering::Relaxed);
                ...
                if result.is_some() {
                    ...
                    if !available.is_empty() {
                        ...
                        self.round_robin_idx
                            .store(available[next_pos], Ordering::Relaxed);
                    }
                }
                result
            }
            PoolStrategy::Random => Self::acquire_random(&creds),
            PoolStrategy::LeastUsed => Self::acquire_least_used(&creds),
        }
    }
