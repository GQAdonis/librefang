...
RUN set -eux; \
    ...
    curl -fsSL "https://go.dev/dl/go1.26.3.linux-${go_arch}.tar.gz" \
    | tar -xz -C /usr/local
...
RUN set -eux; \
    ...
    curl -fsSL \
    "https://github.com/tinygo-org/tinygo/releases/download/v0.41.1/tinygo_0.41.1_${tg_arch}.deb" \
    -o /tmp/tinygo.deb; \
...
RUN set -eux; \
    ...
    curl -fsSL \
    "https://github.com/WebAssembly/wasi-sdk/releases/download/wasi-sdk-27/wasi-sdk-27.0-${ws_arch}-linux.tar.gz" \
    | tar -xz -C /opt; \
    mv "/opt/wasi-sdk-27.0-${ws_arch}-linux" /opt/wasi-sdk
