...
#   BOSSFANG_PREFERRED_VERSION / LIBREFANG_PREFERRED_VERSION
#                                 prefer a version tag, falling back when it ships no package
...
asset_available() {
    _aa_url="https://github.com/$REPO/releases/download/$1/bossfang-$2.tar.gz"
...
# Resolve VERSION+PLATFORM: BOSSFANG_VERSION (or legacy LIBREFANG_VERSION) is a
# hard pin; BOSSFANG_PREFERRED_VERSION (or legacy LIBREFANG_PREFERRED_VERSION) is
# a soft hint with fallback. BOSSFANG_* wins; LIBREFANG_* is the back-compat fallback.
resolve_installable_version() {
    _pinned="${BOSSFANG_VERSION:-${LIBREFANG_VERSION:-}}"
    if [ -n "$_pinned" ]; then
        VERSION="$_pinned"
    ...
    _preferred="${BOSSFANG_PREFERRED_VERSION:-${LIBREFANG_PREFERRED_VERSION:-}}"
...
install() {
    ...
    # The BossFang release tarball packages a single binary named `bossfang`.
    NEW_BIN="$STAGE/bossfang"
    if [ ! -f "$NEW_BIN" ]; then
        echo "  ${C_RED}Archive did not contain the bossfang binary.${C_RESET}"
        exit 1
    fi
    chmod +x "$NEW_BIN"
    ...
    if ! install_binary_with_rollback "$NEW_BIN" "$INSTALL_DIR/bossfang"; then
        print_source_install_hint
        exit 1
    fi

    # Create a `librefang` symlink alongside the verified `bossfang` binary so
    # existing user scripts that invoke `librefang …` keep working. This mirrors
    # the dual `[[bin]]` declaration in librefang-cli/Cargo.toml. `ln -sf`
    # overwrites any prior librefang binary from a pre-rebrand install — that's
    # the migration step.
