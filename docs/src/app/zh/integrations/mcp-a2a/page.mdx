...
BossFang 支持 MCP (Model Context Protocol) 和 A2A (Agent to Agent) 协议。
...
BossFang 本身就是 stdio MCP 客户端，因此两边都不需要新增任何代码 —— 两段配置就是全部集成工作。
...
这恰好够 `everyapi mcp` 自行找到 `~/.config/everyapi/credentials.json`，所以不需要把任何凭据复制进 BossFang 的配置或密钥库。
...
BossFang 会把每个 MCP 工具命名为 `mcp_{server}_{tool}`，而 EveryAPI 的工具**本身**就叫 `everyapi_*`。
两者按字面拼接，不做任何去重：
...
A2A 协议支持 BossFang agents 之间的通信。
...
这些端点用于让 BossFang 作为客户端，主动向外部 A2A agent 委派任务：
...
这些端点由 BossFang 对外暴露，供其他 A2A agent 向本实例发送任务：
