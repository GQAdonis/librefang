...
BossFang CLI 所有命令组的详细参考：Agent、Hand、工作流、触发器、技能、通道和配置。
...
- 从以下位置发现模板：`~/.librefang/workspaces/agents/`（由注册表同步预装）、以及 `LIBREFANG_AGENTS_DIR`（环境变量覆盖）。
- 每个模板是一个包含 `agent.toml` 清单的目录。
  直接放入这些目录的扁平 `<name>.toml` 文件会被跳过，因为发现逻辑先按 `is_dir()` 过滤，之后才会查看任何文件。
- 通过仪表板、`agent_type_create` 工具或 `save-as-agent-type` 创建的 agent 类型改为写入 `~/.librefang/agent-types/<name>.toml`（#7758），而本选择器不会扫描该目录。
  守护进程仍能解析它们——`agent_spawn` 与工作流步骤的 `{"type": ...}` 会优先搜索 `agent-types/`——因此请通过守护进程而非 `agent new` 来生成这类 agent。
...
向 Agent 发送一条消息并打印回复。
与 `agent chat` 不同，这是单次请求，适用于脚本和自动化。
...
| 选项 | 默认值 | 说明 |
|---|---|---|
| `--json` | 关闭 | 打印原始 JSON 响应，而不仅是回复文本。 |
| `--incognito` | 关闭 | 本轮不写入会话与记忆；记忆读取仍然生效。 |
| `--session-id <UUID>` | *（规范会话）* | 指定要对话的具体会话。也可写作 `--session`。 |

**会话定向：**

不传 `--session-id` 时，守护进程解析到该 Agent 的*规范*会话——每个 Agent 只有一个对话。
这对个人 Agent 没有问题，但对同时服务多个互不相关的用户的 Agent 就是错的：所有调用者的对话都落在同一段历史里，于是每位访客的文本都会出现在下一位访客那一轮的模型上下文中，而这段共享记录会在每一轮被重新发送并计费，直到撞上上下文上限。

传入按对话区分的 UUID，每个对话就拥有各自的历史。
相同的 UUID 继续同一个对话，不同的 UUID 开启一个隔离的对话。
这与 `POST /api/agents/{id}/message` 的 `session_id` 字段等价。

`--incognito` 不能替代它：它会完全抑制会话写入，因此单个对话*内部*的连续性也会丢失。

**与 `session_mode` 的关系：**

显式的 `--session-id` 位于内核会话解析阶梯的最顶端，因此在两个方向上都优先于 `agent.toml` 中的 `session_mode`：

| `session_mode` | 不传 `--session-id` | 传 `--session-id` |
|---|---|---|
| `persistent`（默认） | 每次调用都复用该 Agent 唯一的规范会话。 | 改用指定的会话；规范会话不受影响。 |
| `new` | 每次调用都新建一个一次性会话。 | 指定的会话会在多次调用间复用，从而为该对话恢复连续性。 |

会话必须属于被寻址的 Agent。
属于其他 Agent 的 UUID 会被拒绝而不会被读取，因此一个 Agent 的 id 永远无法暴露另一个 Agent 的历史。
格式非法的 UUID 会在发出请求之前由 CLI 拒绝。
...
管理系统启动服务，使 BossFang 在登录或重启时自动启动。
...
- **Windows：** 在 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` 添加 `BossFang` 注册表项。
...
#### `--system`（macOS）：开机启动，而非登录后启动

上述三个平台的默认行为注册的都是*用户级*服务，只有该用户登录后才会启动。
因此一台重启后停在登录界面（或 FileVault 解锁界面）的 Mac 并不会启动守护进程——这对需要常驻运行的部署是个实际问题。

`--system` 改为将 LaunchDaemon 写入 `/Library/LaunchDaemons/ai.librefang.daemon.plist`。
launchd 会在开机时、任何用户登录之前加载该目录。

```bash
sudo librefang service install --system
```

它需要 root 权限，并且必须通过 `sudo` 调用，而不能直接以 root 身份登录后执行：守护进程需要以一个真实账户运行，而 `SUDO_USER` 是唯一能标识该账户的信息。
生成的 job 会把 `UserName` 设为你的账户，`HOME` 指向你的主目录，`LIBREFANG_HOME` 指向 `~/.librefang`，并在安装时把该状态目录及其中的 `daemon.log` 的所有权移交给你——launchd 会在降权之前打开日志文件，若不做这一步，守护进程将无法写入自己的日志。
...
`librefang service status` 会同时报告用户级 LaunchAgent 和开机启动的 LaunchDaemon，无需任何 flag。

`--system` 仅适用于 macOS。在 NixOS 上请使用 `services.librefang` 模块；其他 Linux 发行版可将系统级 unit 写入 `/etc/systemd/system/`，或继续使用用户级服务加 `loginctl enable-linger`。
...
| `librefang uninstall` | 卸载 BossFang。 | `--confirm`、`--keep-config` |

### 目录与注册表

| 命令 | 用途 | 子命令 |
|---|---|---|
| `librefang models` | LLM 模型目录。 | `list`、`aliases`、`providers`、`set`、`connect` |
...
| `librefang message <agent> <text>` | — | 发一次性消息,打印回复。`--json`、`--incognito`、`--session-id <UUID>` — 见 [librefang message](#librefang-message)。 |
