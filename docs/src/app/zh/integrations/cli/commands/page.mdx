...
- 模板来自三个来源，按优先级从高到低：`LIBREFANG_AGENTS_DIR`（环境变量覆盖）、`~/.librefang/agent-types/<name>.toml`（仪表板编辑器与 `agent_type_create` 写入的规范存储）、以及 `~/.librefang/workspaces/agents/`（assistant 为预装，其余由仪表板安装）。前两者与守护进程自身解析模板的顺序一致；该环境变量只是 CLI 侧的覆盖，守护进程从不读取它。
- 模板要么是 `agent-types/` 中的扁平 `<name>.toml`，要么是另外两个来源中包含 `agent.toml` 清单的目录。
...
| `librefang health` | 存活探针；daemon 报告子系统异常时退出码为 1。 | `--json` |
