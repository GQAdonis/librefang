...
| `role_map` | table of string → string | `{}` | IdP 角色/群组声明值 → BossFang 角色（`owner` / `admin` / `user` / `viewer` / `guest`）。详见下文。 |
...
键是 `claim_paths` 解析出的声明值（默认为 `roles` 和 `groups` 两个声明），值是 BossFang 角色，使用与 `[[users]] role` 和 `[channel_role_mapping]` 相同的 `viewer` < `user` < `admin` < `owner` 阶梯。
...
#### `group_map`：把 IdP 群组接到 BossFang 团队上
