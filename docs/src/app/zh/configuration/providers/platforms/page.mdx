...
可用模型取决于你的账户而非 BossFang。
BossFang 通过 EveryAPI CLI 的凭据进程获取当前网关与 relay 凭据，再加载账户的实时模型目录。
...
1. 安装公开的 [EveryAPI CLI](https://github.com/everyapi-ai/everyapi-ai) 并登录：
   ...
   everyapi auth login
   ```
2. 启动 BossFang。它会检测已登录的 EveryAPI CLI，并自动注册托管 provider：
   ```bash
   librefang start
   ```
...
BossFang 在需要凭据时调用 `everyapi auth credential --format=json`，且不会持久化返回的密钥。
...
输出 token 上限是两个网关接口都不提供的唯一一项，因此回退到 BossFang 的内置目录。文本条目缺少任一 token 上限会在目录加载时被丢弃，所以两个上限都无法解析的模型会被跳过并在命令输出中点名，而不是悄悄注册进去。命令还会报告哪些模型借用了上限，以便区分哪些数字来自网关本身。
