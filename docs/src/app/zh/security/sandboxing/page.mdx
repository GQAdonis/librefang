...
在执行任何 shell 命令之前，BossFang 会将命令字符串与 **43 个正则表达式规则**进行匹配，这些规则涵盖 13 类危险操作。此检测在能力检查和污点检查的基础上提供额外的语义安全层。
...
| **服务管理** | 停止或禁用系统服务，以及重启共享的 BossFang 守护进程 | `systemctl stop`、`systemctl disable`、`librefang stop`、`librefang gateway restart` |
