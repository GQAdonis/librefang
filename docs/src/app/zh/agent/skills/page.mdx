...
技能是可插拔的工具包，用于扩展 BossFang 中 Agent 的能力。一个技能将一个或多个工具及其实现打包在一起，让 Agent 能够完成内置工具无法覆盖的任务。本指南涵盖技能创建、清单格式、Python 和 WASM 运行时、发布到 FangHub 以及 CLI 管理。
...
BossFang 提供 61 个专家知识技能，可从仪表盘安装：
...
技能可以在 `skill.toml` 中声明配置变量。Agent 启动时，BossFang 会从用户的 `~/.librefang/config.toml` 中解析每个变量，并把解析得到的值注入到 Agent 的系统提示词中，让技能无需在代码里硬编码密钥或环境相关取值即可使用。
...
1. BossFang 向脚本的 stdin 发送一个 JSON 载荷：
...
WASM 技能在 BossFang 进程内的 `WasmSandbox` 中运行：受能力门控，并对每次调用施加燃料、线性内存和墙钟时间限制。
...
BossFang 可以安装和运行 OpenClaw 格式的技能。技能安装器会自动检测 OpenClaw 技能（通过查找 `package.json` + `index.ts`/`index.js`）并进行转换。
...
如果目录包含 OpenClaw 风格的技能（Node.js 包），BossFang 会：

1. 检测 OpenClaw 格式。
2. 从 `package.json` 生成 `skill.toml` 清单。
3. 将工具名称映射为 BossFang 约定。
4. 将技能复制到 BossFang 技能目录。
