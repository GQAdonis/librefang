...
BossFang 仍然拥有 SurrealDB 行存储、抽取流水线和全部隔离守卫；你接入的后端只决定哪些记忆 ID 距离查询向量最近。
...
`/search` 返回的一切都被当作不可信数据，因为搜索结果是一个来自 BossFang 无法控制的进程的 ID 列表。

- **ID 从 SQLite 水合，绝不从后端的 payload 水合。** 后端返回的 `payload` 和 `metadata` 不会进入 prompt，进入 prompt 的是 BossFang 本地那条 ID 对应的行。
