...
BossFang keeps owning the SurrealDB row store, the extraction pipeline and every isolation guard; the backend you attach decides only which memory IDs are nearest to a query embedding.
...
Everything `/search` returns is treated as untrusted, because the search result is an ID list from a process BossFang does not control.

- **IDs are hydrated from SQLite, never from the backend's payload.** The `payload` and `metadata` a backend returns are not what reaches the prompt; the row BossFang already has under that ID is.
