...
Skills are pluggable tool bundles that extend agent capabilities in BossFang. A skill packages one or more tools with their implementation, letting agents do things that built-in tools do not cover. This guide covers skill creation, the manifest format, Python and WASM runtimes, publishing to FangHub, and CLI management.
...
BossFang provides 61 expert knowledge skills available for installation from the dashboard:
...
Skills can declare configuration variables in `skill.toml`. At agent startup, BossFang resolves each variable from the user's `~/.librefang/config.toml` and injects the resolved values into the agent's system prompt, making them available to the skill without hard-coding secrets or environment-specific values.
...
This works as a **two-party opt-in**: the skill author declares which variables the skill *wants*, and the operator (the person running BossFang) decides which of those requests to *grant*.
...
1. BossFang sends a JSON payload to the script's stdin:
...
WASM skills run inside BossFang's in-process `WasmSandbox`: capability-gated, with per-invocation fuel, linear-memory, and wall-clock limits.
...
FangHub is the community skill marketplace for BossFang.
...
BossFang can install and run OpenClaw-format skills. The skill installer auto-detects OpenClaw skills (by looking for `package.json` + `index.ts`/`index.js`) and converts them.
...
If the directory contains an OpenClaw-style skill (Node.js package), BossFang:

1. Detects the OpenClaw format.
2. Generates a `skill.toml` manifest from `package.json`.
3. Maps tool names to BossFang conventions.
4. Copies the skill to the BossFang skills directory.
...
7. **Test with multiple agents** -- verify your skill works with different agent types and providers.
