...
| **Quit BossFang** | Logs the quit event and calls `app.exit(0)` |

The tray tooltip reads **"BossFang Agent OS"**.

**Left-click on tray icon** shows the main window on macOS and Windows, or toggles its visibility (shows or hides) on Linux.

### Single-Instance Enforcement

On desktop platforms, `tauri-plugin-single-instance` prevents multiple copies of BossFang from running simultaneously. When a second instance attempts to launch, the existing instance's main window is shown, unminimized, and focused:
...
To actually quit, use the **"Quit BossFang"** option in the system tray menu.
...
Check or toggle whether BossFang launches at OS login. Uses `tauri-plugin-autostart` (launchd on macOS, registry on Windows, systemd on Linux).
...
All other BossFang environment variables (API keys, configuration) apply as normal since the desktop app boots the same kernel as the headless daemon.
