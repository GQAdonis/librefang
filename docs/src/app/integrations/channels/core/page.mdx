...
**This is off by default because it changes what BossFang stores, not only what it shows.**
...
**This is off by default because it changes how many people BossFang stores, not just how well it names them.**
...
2. Start (or restart) the BossFang daemon. The sidecar will log a QR code at INFO — scan it with your WeChat app to confirm. After the first QR login, paste the `bot_token` into `WECHAT_BOT_TOKEN` (in `~/.librefang/secrets.env`) to skip the QR flow on subsequent restarts.
...
**UTF-16 code units** rather than bytes or Unicode code points. BossFang measures
...
within `limit` UTF-16 units. BossFang sends the chunks as separate messages in order,
