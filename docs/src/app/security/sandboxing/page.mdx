...
This page covers the runtime controls that constrain what agents can do and how untrusted execution is contained inside BossFang.
...
**Source:** `librefang-runtime/src/dangerous_command.rs`

Before any shell command is executed, BossFang scans the command string
against **43 regular-expression patterns** grouped into 13 categories of
dangerous operations.  This adds a semantic safety layer on top of the
capability and taint checks.

The screen is independent of `exec_policy.mode` and runs under `deny`, `allowlist`, and `full` alike -- `full` disables allowlist *validation*, not this denylist.
...
| **Daemon database mutation** | Writes against the daemon's own `librefang.db`, which holds every agent, session, and approval record on the host | `sqlite3 librefang.db "DELETE FROM …"`, `> librefang.db` |
| **System file overwrites** | Writing directly to critical OS paths | `> /etc/passwd`, `> /etc/shadow`, `> /boot/` |
| **Service management** | Stopping or disabling system services, and bouncing the shared LibreFang daemon | `systemctl stop`, `systemctl disable`, `librefang stop`, `librefang gateway restart` |
...
| **Container privilege escalation** | Entering a container to act outside the agent's own sandbox | `docker exec` |
