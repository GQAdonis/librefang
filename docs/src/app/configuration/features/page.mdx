...
It covers, among others, BossFang daemon lifecycle commands (`librefang stop` / `start` / `restart`, including the `gateway` alias and path-qualified invocations), which bounce every agent and channel adapter sharing the daemon, and mutating SQL or output redirection against the daemon's own `librefang.db`.
...
| `totp_issuer` | string | `"BossFang"` | Issuer name displayed in authenticator apps during enrollment. |
...
| `output_format` | string or null | `null` (→ `"mp3"`) | Default for the `text_to_speech` tool's `output_format` argument, applied to every provider: `"mp3"` or `"ogg_opus"`. `"mp3"` performs no conversion — the provider's own format is kept. A value passed in the tool call wins. |
...
Only `"ogg_opus"` triggers a conversion; `"mp3"` is the name of the no-op and
leaves whatever the provider returned, so a provider emitting Opus still emits
Opus.

Set `output_format = "ogg_opus"` when synthesised audio is sent to a messaging
channel. A voice note has to be Ogg/Opus, and most providers return MP3, which
the channel rejects — while `text_to_speech` itself reports success, because the
file was written. The conversion runs through ffmpeg and falls back to the
provider's own format when ffmpeg is not installed.

This is separate from `[tts.elevenlabs] output_format` below, which is a
provider query parameter with its own vocabulary and already asks ElevenLabs for
Opus directly, so no conversion step is needed there.
...
| `output_format` | string | `"opus_48000_32"` | The `?output_format=` query parameter sent to ElevenLabs. Others: `mp3_44100_128`, `mp3_22050_32`, `opus_24000_32`, `pcm_16000`, `pcm_44100`, `ulaw_8000`. |
...
Containers are destroyed by the reaper, which runs every 60 seconds while `enabled = true` and applies `idle_timeout_secs` and `max_age_secs`, and by daemon shutdown, which destroys every pooled container. There is no separate "session ended" signal in BossFang, so a `session`-scoped container outlives its last tool call until one of those two reaps it — set `idle_timeout_secs` to the window you want that to be.

A call site with no BossFang session — the `POST /api/tools/{name}` bridge, and other out-of-band invocations — cannot honour `scope = "session"`, so it falls back to creating a container for the call and destroying it afterwards rather than widening the container to the whole agent.
...
**What triggers a reconnect.** Two things mark a server as errored and hand it to the health loop.
A failed connect or reconnect handshake does so on the first failure — there is no live connection to preserve.
A server that handshook successfully and *later* broke at the transport level — a tool call that times out, a closed transport, a dead stdio pipe to the child process — is marked errored as soon as the first such failure is observed, but is only torn down and rebuilt after **three consecutive** transport failures with no successful call in between.
One timeout is indistinguishable from a genuinely slow tool, and a reconnect drops the server process along with any state it holds, so a single failure deliberately does not cause one.
A truly wedged server fails every call, so the three arrive as fast as the agent retries.

A tool that returns a normal error — bad arguments, file not found, anything the server answers with a well-formed JSON-RPC error — never counts toward this.
The transport carried that reply, so reconnecting would drop a healthy server.

Both kinds of failure are reflected in `GET /api/mcp/health` (`status`, `last_error`, `consecutive_failures`), and a successful tool call resets the counter and refreshes `last_ok`.
...
The compression algorithm is implemented behind a `ContextEngine` trait. The current implementation uses an LLM-based summarizer. Future versions of BossFang will allow configuring alternative compression algorithms — including DAG-based compression and LCM (Latent Context Modeling) — once those implementations stabilize.
...
BossFang ships an opt-in observability stack you can bring up with one `docker compose` invocation. It captures HTTP requests **and** BossFang's own work — agent turns, tool calls, channel sends, cron fires — as searchable Tempo traces.
