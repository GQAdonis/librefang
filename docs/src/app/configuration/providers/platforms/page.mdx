...
EveryAPI is a managed provider with no fixed model list.
The available models are a property of your account rather than of BossFang.
BossFang asks the EveryAPI CLI credential process for the current gateway and relay credential, then loads the account's live catalog.

**Setup:**
1. Install the public [EveryAPI CLI](https://github.com/everyapi-ai/everyapi-ai) and sign in:
   ```bash
   everyapi auth login
   ```
2. Start BossFang. It detects the authenticated EveryAPI CLI and registers the managed provider automatically:
   ```bash
   librefang start
   ```
3. To connect explicitly or make EveryAPI the default provider, run:
   ```bash
   librefang models connect everyapi --set-default
   ```

For managed discovery, EveryAPI remains the credential authority.
LibreFang invokes `everyapi auth credential --format=json` when it needs a credential and never persists the returned secret.
Older compatible CLI credentials can still be read as a fallback.
Explicit endpoint or environment-variable configuration remains available and takes precedence over managed discovery.
...
It uses the EveryAPI account independently of provider routing, so it works whether or not `models connect everyapi` has run, and connecting the provider does not enable it: see [Connect the EveryAPI MCP server](/integrations/mcp-a2a#connect-the-every-api-mcp-server).
