...
BossFang polls the built-in local providers (`ollama`, `vllm`, `lmstudio`, `lemonade`) every 60 seconds and merges whatever their `/v1/models` endpoint reports into the catalog.
...
BossFang reads that flag from the gateway when the gateway states it.
