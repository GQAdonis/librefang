...
### Model Auto-Discovery for Custom Providers

LibreFang polls the built-in local providers (`ollama`, `vllm`, `lmstudio`, `lemonade`) every 60 seconds and merges whatever their `/v1/models` endpoint reports into the catalog.
A provider registered under any other id — a self-hosted vLLM at `vllm-local`, say — is not polled by default, because an arbitrary OpenAI-compatible endpoint need not serve a model listing.
Opt in per provider with `discover_models`:
...
An opted-in provider joins the same paths a built-in local one already walks: the periodic probe loop, the `POST /api/providers/{name}/test` refresh (whose response then reports `discovered_models`), and the live-model filter applied to `GET /api/models`.
`GET /api/models` queries the listing itself as well, through the same 60-second probe cache that backs the Dashboard's provider grid, so the models a gateway serves are in the list on the very first request instead of only once a background probe tick has happened to land.
A listing that cannot be fetched is not an error: the response falls back to the checked-in catalog and the reason is logged at `debug` level, so a gateway that proxies `/chat/completions` without exposing `/models` behaves exactly as it did before you turned the flag on.
The flag never turns discovery *off* for the built-in ids — those are decided by the id, not by the flag — so an existing install is unaffected.

The Dashboard exposes the same switch as **Auto-discover models** in the Add Provider wizard and in a provider's Configure drawer; the latter calls `PUT /api/providers/{name}/discovery` with `{"discover_models": true}`, which writes the value back into the provider's own TOML.

The write is durable for a provider you created — nothing else owns its file.
For a **registry-shipped** provider it is not: the boot-time registry sync rewrites any file whose content differs from the upstream copy, so a `discover_models` edit to e.g. `providers/openai.toml` applies immediately and is reverted on the next daemon start.
That is only a limitation on paper for the built-in local ids, whose discovery is decided by the id rather than the flag; if you need a registry-shipped remote provider polled permanently, register it under your own id instead of editing the shipped file.

### Discovered Capabilities, and What "No Vision" Means

A discovered model's `supports_vision` decides whether image attachments reach the model at all: a model recorded as text-only has every image block replaced by a placeholder before the request is built, because a text-only OpenAI-compatible endpoint rejects `image_url` content parts with HTTP 400.

LibreFang reads that flag from the gateway when the gateway states it.
LiteLLM and similar proxies expose a per-model `supports_vision` / `supports_function_calling` on `/v1/models` (top level or under `model_info`), and OpenRouter-shaped listings expose `architecture.input_modalities`; an explicit flag is believed in both directions, because the gateway is the authority on its own models.

When nothing states it, the flag is **inferred from the model's name** — and a name you chose (`team-default`, `fast`, an internal ticket id) carries no capability information.
So an inferred value is recorded as a guess rather than a fact, and a guess never costs you an image: the request is sent with the pictures intact, and if the model really is text-only the provider answers with an error you can act on.
`GET /api/models` reports which of the two you have per model, as `vision_support`:

| `vision_support` | Where it came from | What happens to an attached image |
|---|---|---|
| `supported` | The gateway, the registry, or your own override said yes | Sent |
| `unsupported` | One of those said no | Replaced by a placeholder naming the model and the file path, and a `WARN` is logged |
| `unknown` | Nobody said; the value shown in `supports_vision` is inferred from the name | Sent |

If a model of yours lands on `unknown` and you know the answer, state it — `PUT /api/models/overrides/{provider}:{model}` with `{"supports_vision": true}`, or the capability toggles in the Dashboard's model drawer.
An override is a declaration and outranks everything else.
The same applies to a model you add by hand via `POST /api/models/custom`: include `supports_vision` and it is taken as declared, omit it and the model is `unknown` rather than assumed blind.

### API Keys for Keyless Providers

`key_required = false` means a key is *not mandatory*, not that one is refused.
The built-in local providers all declare it false, but a self-hosted vLLM or an Ollama behind a reverse proxy (Open WebUI, LiteLLM) answers 401 without one.
Set the key exactly as for any other provider — `POST /api/providers/vllm/key`, the API-key field in the Dashboard's Configure drawer, or the declared `api_key_env` in the environment — and the runtime sends it as `Authorization: Bearer` on both the probe and the chat call.
...
A model's own configuration — including its **context window** — is editable at any time, not only when the model is first registered.
Overrides are keyed by `provider:model_id` and stored in `~/.librefang/data/model_overrides.json`, separately from the catalog entry, so a registry sync that rewrites the entry leaves them intact.
...
`PUT` takes the whole document, not a patch — a field omitted from the body is cleared.
...
The two capacity limits answer a different question from the inference parameters next to them.
`context_window` and `max_output_tokens` correct what the model *is* — facts a gateway in front of a self-hosted runtime routinely reports as `null` — while `max_tokens` is the per-request output cap sent on the wire.
Setting the output cap to the model's window asks the model to reserve its entire context for the reply, which is the confusion these fields exist to end.

### Context window precedence

1. `agent.toml` — `[model] context_window`, the most specific layer: one agent stating its own window.
2. `model_overrides.json` — `context_window`, the operator's correction for the model, inherited by every agent that uses it.
3. The catalog entry — whatever the registry declared, or a live `/models` discovery pass assumed.
4. `8192` — the conservative fallback the agent loop applies when nothing above resolves, with a `WARN` naming the value it assumed.

A zero is treated as "unknown" at every layer, never as a limit, so clearing a field falls through to the next layer instead of pinning the window to zero tokens.

`GET /api/models` and `GET /api/models/{id}` report the **effective** `context_window` / `max_output_tokens`, with the raw catalog values preserved under `limits_catalog` so a revert-to-default control has a target to name.

---

## Model Routing

BossFang can automatically select the cheapest model capable of handling each query. This is configured per-agent via `ModelRoutingConfig`.
...
Every signal is a property of the request. Tool count and system-prompt length were scored until #7952 and are not any more: they are the same on every turn of a given agent, so they shifted the whole distribution instead of separating easy turns from hard ones.
An agent with ~50 MCP tools scored +1000 from tool count before a single character of the message was read, past any usable `complex_threshold`, so routing degenerated into pinned-complex and saved nothing.
If your agent's turns now score lower than they used to, lower `simple_threshold` / `complex_threshold` — the scores are no longer inflated by a constant.
...
BossFang tracks the cost of every LLM call and can enforce per-agent spending quotas.
...
A `FallbackChain` lets you configure an ordered list of LLM providers that BossFang
...
BossFang classifies each failure before deciding whether to fall forward:
...
BossFang works through the list in order. If all providers fail for eligible reasons,
...
After every LLM API response, BossFang parses the provider's rate-limit headers into a
...
BossFang recognizes **21 header variants** across major providers:
