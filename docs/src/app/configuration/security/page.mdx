...
| `role_map` | table of string → string | `{}` | IdP role/group claim value → BossFang role (`owner` / `admin` / `user` / `viewer` / `guest`). See below. |
...
The keys are values of whichever claims `claim_paths` resolves — the `roles` and `groups` claims by default — and the values are BossFang roles in the same `viewer` < `user` < `admin` < `owner` ladder that `[[users]] role` and `[channel_role_mapping]` use.
...
#### `group_map`: putting an IdP group on a BossFang team
